...
# Convex Hull 생성을 위한 Monotone Chain 알고리즘
...
def collide_poly_poly(poly1, poly2):
    axes = get_polygon_axes(poly1) + get_polygon_axes(poly2)
    for ax in axes:
        min1, max1 = project_polygon(poly1, ax)
        min2, max2 = project_polygon(poly2, ax)
        if not check_overlap(min1, max1, min2, max2):
            return False
    return True

def collide_poly_circle(poly, center, radius):
    axes = get_polygon_axes(poly)
    ...
    for ax in axes:
        min1, max1 = project_polygon(poly, ax)
        min2, max2 = project_circle(center, radius, ax)
        if not check_overlap(min1, max1, min2, max2):
            return False
    return True

def check_collision(obj1, obj2):
    t1, s1 = obj1.get_shape()
    t2, s2 = obj2.get_shape()
    
    if t1 == 'poly' and t2 == 'poly':
        return collide_poly_poly(s1, s2)
    elif t1 == 'circle' and t2 == 'circle':
        c1, r1 = s1
        c2, r2 = s2
        return math.hypot(c1[0] - c2[0], c1[1] - c2[1]) <= r1 + r2
    elif t1 == 'poly' and t2 == 'circle':
        return collide_poly_circle(s1, s2[0], s2[1])
    elif t1 == 'circle' and t2 == 'poly':
        return collide_poly_circle(s2, s1[0], s1[1])
    return False

# ── 오브젝트 클래스 ─────────────────────────────────────────────────────────────
class DraggableObject:
    def __init__(self, name, surface, x, y):
        self.name = name
        self.image = surface
        ...
        # 각 객체별 Bounding Box 세팅
        if name == "rocket":
            mask = pygame.mask.from_surface(self.image)
            outline_points = mask.outline()
            self.base_hull = get_convex_hull(outline_points)
        elif name == "sword":
            self.angle = 0
    ...
    def get_shape(self):
        if self.name == "adventurer":
            # AABB
            r = self.rect
            return 'poly', [(r.left, r.top), (r.right, r.top), (r.right, r.bottom), (r.left, r.bottom)]
        elif self.name == "stone":
            # Circle
            r = (self.rect.width + self.rect.height) / 4
            return 'circle', (self.rect.center, r)
        elif self.name == "rocket":
            # Convex Hull
            return 'poly', [(self.rect.x + px, self.rect.y + py) for px, py in self.base_hull]
            ...
            poly = []
            for dx, dy in self.base_corners:
                # 파이게임 회전 방식(y축이 아래를 향함)을 반영한 회전 변환
                nx = dx * cos_a + dy * sin_a
                ny = -dx * sin_a + dy * cos_a
                poly.append((cx + nx, cy + ny))
            return 'poly', poly
    ...
    def draw_bounding_box(self, surface):
        color = (0, 0, 255) if self.is_colliding else (255, 0, 0)
        t, s = self.get_shape()
        if t == 'poly':
            pygame.draw.polygon(surface, color, s, 2)
        elif t == 'circle':
            pygame.draw.circle(surface, color, (int(s[0][0]), int(s[0][1])), int(s[1]), 2)
...
if __name__ == "__main__":
    ...
    pygame.display.set_caption("Sprites Collision & Interaction")
    ...
    for i, name in enumerate(names):
        ...
        surf = fit_surface(load_sprite(name), BOX_W, BOX_H)
        sw, sh = surf.get_size()
        x = box_x + (BOX_W - sw) // 2
        y = PADDING + (BOX_H - sh) // 2
        objects.append(DraggableObject(name, surf, x, y))
    ...
    while running:
        ...
        for obj in objects:
            obj.draw(screen)
            obj.draw_bounding_box(screen)

        pygame.display.flip()
        clock.tick(60)  # 요구사항: 60 FPS
